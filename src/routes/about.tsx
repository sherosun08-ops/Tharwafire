...
import { Shield, Award, Users, TrendingUp, Globe, Star, Loader2 } from "lucide-react";
...
function About() {
  ...
  // Get data from CMS settings or fall back to defaults
  const timelineData = settings?.about_timeline
    ? (isAr ? (settings.about_timeline as {year: string; title: string; desc: string}[]) || defaultTimelineAr : (settings.about_timeline_en as {year: string; title: string; desc: string}[]) || defaultTimelineEn)
    : (isAr ? defaultTimelineAr : defaultTimelineEn);

  const teamData = settings?.about_team
    ? (isAr ? (settings.about_team as {name: string; role: string; bio: string; ab: string}[]) || defaultTeamAr : (settings.about_team_en as {name: string; role: string; bio: string; ab: string}[]) || defaultTeamEn)
    : (isAr ? defaultTeamAr : defaultTeamEn);

  const valuesData = settings?.about_values
    ? (isAr ? (settings.about_values as {icon: string; title: string; desc: string}[]) || defaultValuesAr : (settings.about_values_en as {icon: string; title: string; desc: string}[]) || defaultValuesEn)
    : (isAr ? defaultValuesAr : defaultValuesEn);

  // Map icon names to components
  const iconMap: Record<string, typeof Shield> = { Shield, Award, Users, Globe };
  const values = valuesData.map(v => ({
    ...v,
    icon: iconMap[v.icon as string] || Shield
  }));

  // Get hero stats from settings or use defaults
  const heroStats = [
    { icon: TrendingUp, v: settings?.about_stat1_value || "$2B+", l: t('about_stat1') },
    { icon: Users, v: settings?.about_stat2_value || "5,000+", l: t('about_stat2') },
    { icon: Star, v: settings?.about_stat3_value || "98.5%", l: t('about_stat3') },
    { icon: Globe, v: settings?.about_stat4_value || "15+", l: t('about_stat4') },
  ];

  // Get CMS hero content
  const heroTitle = isAr ? (settings?.about_hero_title || t('about_heading')) : (settings?.about_hero_title_en || t('about_heading'));
  const heroTitleGold = isAr ? (settings?.about_hero_title_gold || t('about_heading_gold')) : (settings?.about_hero_title_gold_en || t('about_heading_gold'));
  const heroTitleEnd = isAr ? (settings?.about_hero_title_end || t('about_heading_end')) : (settings?.about_hero_title_end_en || "");
  const heroDesc = isAr ? (settings?.about_hero_desc || t('about_desc')) : (settings?.about_hero_desc_en || t('about_desc'));
      ...
      <section className="relative py-28 bg-gradient-hero overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8 text-center">
          ...
          <h1 className="mt-5 text-5xl md:text-6xl font-black text-foreground leading-tight">
            {heroTitle} <span className="text-gradient-gold">{heroTitleGold}</span> {heroTitleEnd}
          </h1>
          <p className="mt-6 max-w-2xl mx-auto text-lg text-text-muted leading-relaxed">{heroDesc}</p>
